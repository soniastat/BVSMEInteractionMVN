...
# Normal prior model for multivariate normal response (NP-MVN)
...
# Update theta
update_theta_NP_MVN <- function(Y, K, W, n_all_par, J, M, Sigma_update)
{
  ...
  V_theta <- as.matrix(bdiag(var_alpha_0, var_beta, var_gamma, var_delta))
...
# Update Sigma
update_Sigma_NP_MVN <- function(Y, n, W, theta_update, Psi_0, nu_0)
{
  # Residuals
...
#' Update parameters for NP-MVN model
#' @export
                            ...
                            nu_0, Psi_0)
{
  ...
  for(s in 2:niter)
  {
    ...
    # theta_update
    theta_update_s <- update_theta_NP_MVN(Y, K, W, n_all_par, J, M,
    ...
    # Sigma_update
    Sigma_update_s <- update_Sigma_NP_MVN(Y, n, W, theta_update[s, , ], Psi_0, nu_0)
