...
source("/projects/soniast@colostate.edu/alpine_sim_NHRHSSI_modify4.R")
...
sim_data <- Sim_data_BVS_real_sim_var_elevated(
  K = K,
  n = n,
  J = J,
  M = M,
...
theta_update_save8 <- tryCatch({
  ...
  res <- fit_NHRHSSI_MVN_cov_modify3(
...
dir.create("/projects/soniast@colostate.edu/results_NHRHSSI_modify4_allvarsim_elevated", recursive=TRUE, showWarnings=FALSE)

outfile <- paste0("/projects/soniast@colostate.edu/results_NHRHSSI_modify4_allvarsim_elevated/modify4_allvarsim_elevated_NHRHSSI_cov_sample_", sample_id, ".rds")
