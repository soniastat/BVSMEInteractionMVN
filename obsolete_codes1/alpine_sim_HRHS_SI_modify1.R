...
# Functions for simulating data and fitting BVS-HRHS-SI-MVN-cov model
...
update_psi_beta_BVS_HRHS_SI_MVN_cov1 <- function(J, lambdasq_beta_update)
{
  rinvgamma(
    n = J,
    shape = 1,
...
update_psi_gamma_BVS_HRHS_SI_MVN_cov1 <- function(M, lambdasq_gamma_update)
{
  rinvgamma(
    n = M,
    shape = 1,
...
update_psi_delta_BVS_HRHS_SI_MVN_cov1 <- function(J, M, lambdasq_delta_update)
{
  rinvgamma(
    n = J * M,
    shape = 1,
...
fit_BVS_HRHS_SI_MVN_cov_modify1 <- function(niter = 20, burn_in = 2, thin = 1,
                                    ...
                                    sigmasq_varphi = 10)
{
  ...
  for(s in 2:niter)
  {
    ...
    # Update psi_beta
    psi_beta_update_s <- update_psi_beta_BVS_HRHS_SI_MVN_cov1(J,
                                                             lambdasq_beta_update = lambdasq_beta_update[s, ])
    psi_beta_update[s, ] <- psi_beta_update_s


    # Update psi_gamma
    psi_gamma_update_s <- update_psi_gamma_BVS_HRHS_SI_MVN_cov1(M,
                                                               lambdasq_gamma_update = lambdasq_gamma_update[s, ])
    psi_gamma_update[s, ] <- psi_gamma_update_s


    # Update psi_delta
    psi_delta_update_s <- update_psi_delta_BVS_HRHS_SI_MVN_cov1(J, M,
                                                               lambdasq_delta_update = lambdasq_delta_update[s, ])
